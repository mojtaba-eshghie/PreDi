        ...
        for index, (predicate1, predicate2) in enumerate(predicates):
            with self.subTest(index=index, predicate1=predicate1, predicate2=predicate2):
                try:
                    ...
                    debug_print(f"Comparing predicates at index {index} resulted in: {result}")
                    self.success_count += 1
                except Exception as e:
                    debug_print(f"Exception occurred at index {index} while comparing '{predicate1}' with '{predicate2}': {e}", is_exception=True)
